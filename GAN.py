...
class GAN(object):
    ...
    def build_model(self):
        ...
        """ Loss Function """
        G = self.generator(self.noise, reuse=False)
        D_real = self.discriminator(self.inputs, reuse=False)
        ...
        self.g_optim = tf.train.AdamOptimizer(0.0002, beta1=0.5).minimize(self.g_loss, var_list=g_vars)

        """ Testing """
        self.sample_noise = np.random.uniform(-1, 1, size=(self.batch_size, self.noise_dim))
        self.fake_images = self.generator(self.noise, reuse=True)

    def discriminator(self, image, reuse=False):
        with tf.variable_scope('discriminator', reuse=reuse):
            image = tf.reshape(image, [self.batch_size, -1])

            # Hidden Layer
            net = tf.nn.relu(
                self.linear(image, 128, scope='d_linear_1'))

            # Output Layer
            out = self.linear(net, 1, scope='d_linear_2')
    ...
    def generator(self, noise, reuse=False):
        with tf.variable_scope('generator', reuse=reuse):
            # Hidden Layer
            net = tf.nn.relu(
                self.linear(noise, 128, scope='g_linear_1'))

            # Output Layer
            out = tf.nn.sigmoid(
                self.linear(net, self.img_height * self.img_height * self.img_color_dim, scope='g_linear_2'))

            return tf.reshape(out, [self.batch_size, self.img_height, self.img_width, self.img_color_dim])
    ...
    def train(self, epochs):
        ...
        for epoch in xrange(epochs):
            ...
            for index in xrange(0, self.batch_index):
                ...
                batch_noise = np.random.uniform(-1, 1, size=(self.batch_size, self.noise_dim))
